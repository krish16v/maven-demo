package com.gontusoftware.demos;
...
public class IRSChainCode {
	...
	if (!"Reuters".equalsIgnoreCase(proposingNode)) {
	...
	BigDecimal floatInterest = findInterestRate(ctx, contract.getFloatingRateIndex(), contract.getCouponFrequency(),
		nextPaymentDate);
	if (floatInterest == null) {
	    throw new IllegalStateException("No Interest Rate for " + contract.getFloatingRateIndex() + ":"
		    + contract.getCouponFrequency() + ":" + nextPaymentDate);
	}
    ...
    private BigDecimal findInterestRate(ThetaContext ctx, String indexName, String period, Date validForDate)
	    throws SQLException {
